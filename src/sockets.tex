...
We are going to define a \ml+client+ command such that \ml+client host
port+ establishes a connection with a service numbered \ml+port+ on
the machine named \ml+host+, then sends on the connection everything
it reads from its standard input, and writes on its standard output
everything it receives on the connection.
