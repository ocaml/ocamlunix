...
read connection requests on a particular pipe. The requests would
...
\end{listingcodefile}
%
The argument is the name of a protocol and the result a record 
...
The \ml+netstat+ Unix command lists the current connections and their
status.
...
\section{Complete example: the universal client\label{sec/univclient}}
...
\textsc{http} command requesting the web page \ml+/~remy/+ on this server.
...
The reusable function \ml+Misc.retransmit fdin fdout+ reads data on the
...
a stream type socket in the Internet domain with the default protocol
...
The connection is closed either by the client or the server:
\begin{itemize}
...
\item If the parent or child client exits prematurely the socket will
  ...
  side of the socket, which is eventually detected by the other part of the
  client. 
\end{itemize}


\section{Establishing a service\label{sec/service}}
...
to bind. The constant address \libvalue{Unix}{inet\_addr\_any} can be
...
\end{listingcodefile}
The function \ml+Misc.install_tcp_server addr+ creates a stream type socket
in the Internet domain with the default protocol, prepares it to
accept new connection requests on the address \ml+addr+ with \ml+bind+
and \ml+listen+. Given that this is a reusable function, we close the
socket cleanly in case of an error.
...
\end{listingcodefile}
The \ml+Misc.tcp_server+ function creates a socket with
...
reusable function we restart the \ml+accept+ call if it is
...
following function reusable function captures this pattern:
\begin{codefile}{misc.mli}
...
connection and the parent process immediately calls \ml+accept+ again. We
obtain the following reusable function:
%
\begin{codefile}{misc.mli}
val fork_treatment :
 file_descr -> (file_descr * sockaddr -> unit) ->
...
\end{listingcodefile}
% 
Note that is is essential that the parent closes \ml+client_sock+
...
which the connection request was received. For one, it does not need
it.  For another, the server may stop accepting new connections before
...
using a double fork (see section \ref{double-fork}).
%
\begin{codefile}{misc.mli}
...
group can be killed to terminate the service. For this reason servers
...
\ml+free_children+ on page~\pageref{ex/childs}).
...
documentation of \libvalue{Unix}{getsockopt} function and its variants
...
\begin{example} 
The following two parameters apply only to stream type sockets in the
Internet domain.
...
negotiates the disconnection. The code below turns \ml+close+
into a blocking operation on the socket \ml+sock+. It blocks either
...
This command is \quotes{universal} server in the sense that it factors
...
(line~\ref{prog:gethost}) is by calling \ml+gethostname+. But in
general many addresses are referencing the same machine. For instance,
the address of the \ml+pauillac+ machine is \ml+128.93.11.35+, it can
also be accessed locally (provided we are already on the
\ml+pauillac+ machine) with the address \ml+127.0.0.1+. To provide a
service on all the addresses pointing to this machine, we can use the
constant address \libvalue{Unix}{inet\_addr\_any}. 

The service is handled by a \quotes{double fork}. The \ml+service+
function redirects standard input and the two standard output on the
connection socket and executes the requested command (note that the
handling of the service cannot be done sequentially).
...
\item The client ends prematurely and closes the connection in the
  ...
  acceptable since nothing is reading the output of this command.
...
nothing about its clients and in particular if the service is public
...
elegant, it still needs a timeout, for example by programming an alarm
(see section~\ref{sec/usingsignals}).


\section{Communication in connectionless mode\label{sec/mode-deconnecte}}
...
\end{listingcodefile}
% 
Each of these calls returns the size of the transferred data. The
...
address like \ml+0+. In contrast, doing this with a stream type socket
...
\begin{mltypecases}
\begin{tabular}{@{}ll}
\ml+MSG_OOB+ & To send an exceptional value. \\
\ml+MSG_DONTROUTE+ & Indicates short-circuiting the default routing
table. \\
%
...
\end{listingcodefile}
% 
The \ml+open_connection+ function opens a socket to the given address
and creates a pair of \ml+Pervasives+ input/output channels on this
...
that a request has been entirely sent. The client can shutdown the
...
bytes. The client and the server must then agree on the precise
transmission protocol, which specifies the format of requests and
responses exchanged on the socket. Most protocols used by Unix
...
The {\ocaml} system helps to encode and decode data structures (called
...
on the content of the channel being read at runtime. Type-checking
...
command's argument.  Responses are also made of one or more lines of
...
focus on the essential, namely the architecture of clients or relays,
...
\subsection*{{\normalfont\http} relay\label{sec/relay}}
...
than directly to the individual {\http} servers located
...
(or if omitted, the default port for {\http}). We reuse the code of
...
(\eg{} if a client requesting a web page with images, it will
...
by an end of file, since we know its size and the connection can stay
...
\end{lstlisting}
% 
Superfluous spaces are also allowed around the \ml+':'+ separator. Any
...
The \textit{\ml{body}} of the response transmitted in a
...
\end{itemize}
%
Here is an example of an single block response:
\begin{lstlisting}
HTTP/1.1 200 OK\r
