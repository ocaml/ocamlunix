...
transmit data. Here are three examples of possible modes and their
properties:
%
\begin{center}
\begin{tabular}{lll}
Mode & Reliable & Data representation \\
\hline
...
transmit unstructured byte sequences (\eg{} \ml+rsh+).  The
...
(\eg{} \ml+biff+); others, to get more network performance, with the
...
Several socket operations use socket addresses. They are values of the
variant type \ml+sockaddr+:
\begin{lstlisting}
...
\end{listingcodefile}
%
The first argument is the service name (\ie{} \ml+"ftp"+ for FTP servers,
...
which kills it. It is the expected behavior. However, the client dies
...
\end{lstlisting}
%
This option turns \ml+close+ into a blocking operation on the
...
disconnected by connecting them to an invalid address, \eg{} \ml+0+.  By
contrast, this would generally result in an error on a socket of
\ml+SOCK_STREAM+ type.

\subsection*{Low level reads and writes}

The \syscall{recv} and \syscall{send} generalize the \indexvalue{read} and
...
\end{mltypecases}
% 
These primitives may be used in connection-oriented mode instead of
...
channels (which will close the socket) or more \quotes{cleanly} by calling
...
writing. Once the service has been provided (\ie{} once \ml+f+ has
...
terminates, except in case of an error (\eg{} of the {\ocaml} runtime
...
order to minimize the conversion work needed, and to make most of the
...
and circularities inside transmitted objects; and they can communicate between
architectures of different \emph{endianness}.

\begin{example}
If \textsc{x}-window were written in {\ocaml}, we would have a variant type
\ml+request+ representing requests that can be sent to the server, and another
variant type \ml+reply+ of eventual server responses:
%
\begin{lstlisting}
type request =
    ...
...
\ml+-N+ & To not download the {\URL} if the file \ml+./mi/ui+
does not exist or is older than the {\URL}. \\
\ml+-r+ & To download recursively all the {\URL}s embedded in the
