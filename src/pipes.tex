...
So far we learned how to manage processes and how they can communicate
with the environment by using files. In the remaining part of the
course we look how processes running in parallel can cooperate by
...
which a process writes data and the other reads it. If a file is used
as the communication medium, the reader can detect that the file
doesn't grow anymore (\ml+read+ returns zero) but it doesn't know if
that means that the writer is finished or if it is only busy computing
more data. Moreover the file keeps track of all the data transmitted
which can be problematic, especially regarding disk space usage.

Pipes provide a mechanism suitable for that kind of communication. A
pipe is made of two file descriptors. The first one, open in read mode
represents the pipe's output. The second one, open in write mode is
...
input. The pipe itself is an internal object of the kernel which can
...
A pipe behaves like a queue (\emph{first-in, first-out}), what is read
on the pipe's output is what was written on its input in the same
order. Writes (calls to \indexvalue{write} on the pipe's input
...
and return when all the data given to \ml+write+ has been
transmitted. Reads (calls to \indexvalue{read} on the pipe's output
descriptor) empty the pipe. If the pipe is empty, a call to \ml+read+
...
the two file descriptors of the pipe, like any other file descriptor,
...
\node (outfather) at (-2,1.5) [fd] {\texttt{fd\_out} father};
\node (outchild) at (-2,-1.5) [fd] {\texttt{fd\_out} child};
\node (infather) at (2,1.5) [fd] {\texttt{fd\_in} father};
...
\end{myimage}
% 
Data written by the child on \ml+fd_out+ is transmitted to the
...
implementation each process first reads $k$ primes $p_1, \ldots, p_k$
on its input before transforming itself in a filter that eliminate the
multiples of $p_1, \ldots, p_k$. In practice $k = 1000$ gives a
...
(the actual representation is unspecified but it is guaranteed to be platform
independent and unique for a version of the language); second, since
these functions use buffered \textsc{i/o}, less system calls are
...
\end{listingcodefile}
%
They allow to perform buffered \textsc{i/o} on descriptors acquired
...
\textsc{i/o} which is possible but very brittle and highly
discouraged~---~in particular for input. Note also that it is possible
...
father reads numbers on its input and writes them to the pipe if they
are not a multiple of one the 1000 primes it initially read.
...
Hence when \ml+k+ is given, the father will terminate first and close
...
If \ml+k+ is not given, all processes continue infinitely until one or
more are killed. The death of a process results in the death its child
...
father, otherwise the child will wait infinitely for new integers from the
father and this leads to a deadlock (closing the channel empties the
...
\ml+fork+. For that all is needed is to add 
...
\end{listingcodefile}
%
The first argument is the name of the pipe, the second one the
requested access permissions. 

Named pipes are opened with a call to \syscall{openfile} like any
...
process for writing (resp. reading); if this already happened,
there's no blocking. Blocking can be avoided altogether by opening the
pipe with the flag \ml+O_NONBLOCK+. But in that case reads and writes
on the pipe won't block either. This flag can be changed after the
pipe was opened with the function \ml+clear_nonblock+ to make further
reads or writes on the pipe blocking, or non blocking with
\ml+set_nonblock+.
%
\begin{listingcodefile}{tmpunix.mli}
...
named piped) are \quotes{new} descriptors, distinct from \ml+stdin+,
...
(read: \quotes{\emph{dup}licate a descriptor \emph{to} another
  descriptor}) which allows to redirect a descriptor on another. This
can be done because there's a level of indirection between a file
descriptor (an object of type \ml+file_descr+) and the object in the
kernel called a \emph{file table entry} that points on the actual
...
\end{listingcodefile}
%
The effect of \ml+dup2 fd1 fd2+ is to point the descriptor \ml+fd2+ on
the file table entry pointed by \ml+fd1+. After the call, these two
...
\end{lstlisting}
% 
After the call to \ml+dup2+, the descriptor \ml+stdin+ points on the
...
terminates, all its open descriptors are closed.  As such there's no
...
\begin{answer}
\begin{itemize}
\item For \ml+>>+ we do like the \ml+>+ redirection, except that the
file  is opened with the flags \ml+[O_WRONLY; O_APPEND; O_CREAT]+.
%
\item For \ml+2>+, we do like the \ml+>+ redirection, except that
\ml+dup2 fd stderr+ is executed instead of \ml+dup2 fd stdout+
%
\item For \ml+2>1+, just call \ml+dup2 stderr stdout+ before executing
the command.
%
\item For \ml+<<+ the shell \ml+sh+ creates a temporary file in
\ml+/tmp+ containing the lines that follow \ml+<<+ and executes the 
command with its standard input redirected on this file. Another 
...
\ml+fd1+ and \ml+fd2+ already point on the same file table entry.  The
...
\node (composef) at (9.5,0) [process] {\texttt{compose} \scriptsize{(father)}};
...
\end{myimage}
%
the father calls \ml+dup2+, we get~:
%
\begin{myimage}[width="100\%"]
\begin{tikzpicture}
\node (ldots) at (0, 0) {...};
...
\end{myimage}
%
the child calls \ml+dup2+ and \ml+execv+, we get~:
\begin{myimage}[width="100\%"]
\begin{tikzpicture}
\node (ldots) at (0, 0) {...};
\node (cmd) at (1.5, 0) [process] {\texttt{cmd}$_{i-1}$};
...
\end{myimage}
%
and everything's ready for the next iteration. 
...
child. Hence it sufficient to \ml+fork+ and \ml+exec+. The father then
...
prevents us from having to parse complex commands into tokens like
...
\end{lstlisting}
% 
We could have done that ourselves like in the mini-shell example, but
it is painful.

\section{Input/output multiplexing}

In all the examples so far, processes communicate \quotes{linearly}~:
each process reads data coming from \emph{at most one} other process. In this
...
windows and processes, we need to interleave the data transmissions.
...
try to read the output of the shell but that it has nothing to
display at that moment the multiplexer process will block. And if at
the same time the editor displays characters, these won't be sent to
the client. There's no way to know in advance on which pipes there is
...
called \quotes{starvation}).

Here is another approach~: we associate to each user process a
\quotes{repeater} process. The repeater reads the output of the pipe
...
algorithms we says that we need to enforce the mutual exclusion of
...
A solution that works is to take the first approach (a single
...
processing time even if there's nothing to do, even if no process is
sending data. This can be alleviated by introducing calls to
\ml+sleep+ in the reading loop however it is very difficult to find
the right frequency. The frequency that doesn't load the machine when
there's nothing to do but that introduces no perceptible delay in
transmissions when there's a lot of data to send.

This is a serious problem. To solve it the designers of \textsc{bsd}
Unix introduced a new system call, \syscall{select}, which is now
available on most Unix variants. A call to \ml+select+ allows a
process to wait (passively) on one or more input/output events.
An event can be~:
%
\begin{itemize}
\item A read event~: \quotes{there's data to read on that descriptor}.

\item A write event~: \quotes{it is possible to write on that descriptor
  without blocking}.

\item An exceptional event~: \quotes{an exceptional condition is
  true on that descriptor}. For example on certain network connections
  high-priority data can be sent (\emph{out-of-band data}) which
  overtakes normal data waiting to be sent. Receiving that kind of 
...
event occurred the three lists are empty.
...
descriptors (\ml+input_fds+) that contains the input descriptors
...
we do not limit the waiting time. When \ml+select+ returns, we test if
there's data waiting on an input descriptor or on the serial port.

If there's data on an input descriptor we \ml+read+ this input in a
...
message with zero bytes to be interpreted as \quotes{the user process
...
\ml+End_of_file+, that is when the end-of-file is reached on the
serial port.
\end{example}
...
\ml+write+ can be interrupted by a signal. In that case the {\ocaml}
call to \ml+Unix.write+ is interrupted and the error \ml+EINTR+ is
raised. The problem is that part of the data may already have been
...
10\% (and not 50\% as one may think) because the overall cost of the
...
call (detected by a negative return value) it is propagated to
...
\end{listingcodefile}
%
This function is already available in the \ml+Unix+ module since version
\texttt{3.08}. But if we had done the work above ourselves we would
need to compile it as follows to use it (assume the {\ocaml} code is
...
system call \syscall{write}. The only remaining difference is when the
...
guaranteed for long writes. This difference is marginal but one should
be aware of it.
