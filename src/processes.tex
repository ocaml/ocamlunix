...
A process is a running program.  A process consists of a program
(machine code) and a state of the program (current control point,
variable values, call stack, open file descriptors, etc.).
...
\end{listingcodefile}
%
The new (\quotes{child}) process is a nearly perfect clone of the one
which called \ml+fork+ (\quotes{parent}).  Both processes execute the
same program (code?), are initially at the same control point (the
return from \ml+fork+), attribute the same values to all variables,
have identical call stacks, and hold open the same file
descriptors to the same files.  The only thing which distinguishes
the two processes is the return value from \ml+fork+:
zero in the child processes, and a non-zero integer in
the parent.  By checking the return value from \ml+fork+, a
program can thus determine if it's in the parent process or the child
and behave accordingly in each:
%
\begin{lstlisting}
match fork() with
| 0 -> (* code run only in the child  *)
| _ -> (* code run only in the parent *)
\end{lstlisting}
%
The non-zero integer returned by \ml+fork+ in the parent process is
the process id of the child.  Each process is identified in
the kernel by a number, the (\emph{process id}).  A process can obtain
...
\ml+fork+, each process modifies independently its \quotes{own}
reference, without affecting the other process.

Similiarly, the open file descriptors are copied at the moment of the
...
its parent thus act like the argument and result descriptors after a
\ml+dup+, but they're in different processes as opposed to the same
one.)
...
(ligne~\ref{prog:delay}).  (The \ml+time+ call returns the current
...
For its first argument, the command \ml+waitpid+ accepts two optional
flags: The flag \ml+WNOHANG+ indicates not to wait, if there's a child
that responds to the request but has not yet terminated. In that case,
the first result is \ml+0+ and the second undefined. The flag
\ml+WUNTRACED+ indicates to return equally (???) the child processes
which have been halted by the signal \ml+sigstop+.  The command raises
the exception \ml+ECHILD+ if no child process matches \ml+p+ (in
particular, if \ml+p+ is \ml+-1+ and the current process has no more
child processes).

\begin{example}
\label{ex/forksearch}
The function \ml+fork_search+ below performs a linear search over
a vecteur, in two processes. It relies on the function
\ml+simple_search+ to perform the simple linear search.
%
...
processes.  When a process terminates, it passes into a state called
\quotes{zombie}, where most of the resources it uses (memory, etc.)
have been freed, but not all: it continues to occupy a slot in the
process table, in order to transmit its return value to the parent via
the call \ml+wait+. Only once the parent calls \ml+wait+ is the zombie
process removed from the process table. Since this table is of fixed
size, it's important, in order to avoid leaks, to call \ml+wait+ on
each process one forks.

If the parent process terinates before the child, the child is given
the process number~$1$ (\ml+init+) as parent. This process contains
an infinite loop of \ml+wait+ calls, and will therefore make the child
process disappear once it finishes. This leads to a useful technique
if you cannot easily call \ml+wait+ on each process you create
(because you cannot afford to block on termination of the child process,
for example): the technique \quotes{du double \ml+fork+}.
%
\begin{lstlisting}
match fork () with
...
The process which calls \ml+exec+ is the same one which executes the
...
line into words, then we \ml+fork+.  The child process uses
