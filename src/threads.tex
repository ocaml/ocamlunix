...
how to use simulated (``VM-level'') threads.  The text and examples in
...
that that creates a process clone with a function that creates a
...
% Note that the original says ``non vide'' (non-empty)
% in the following sentence, which in incorrect.
...
% shoudn't prog:slock refer to the following line (the Condition.wait)?
...
sent its signal.
% The original says $p$ in the following, but I think that's incorrect.
...
threads always have the same view of memory, except perhaps when the
