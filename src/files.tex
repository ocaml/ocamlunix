...
%
% Translation by Richard Paradies, reworked by Daniel C. Bünzli
%------------------------------------------------------------------------------
...
\item \ml+O_EXCL+, \ml+openfile+ fails if the file already exists.
  ...
  files as \emph{locks}\label{page/lock}
  \footnote{This is not possible if the lock file
...
To illustrate the bufferized \io techniques, we implement a fragment
...
\end{listingcodefile}
%
To read a character from an \ml+in_channel+, we do one of two things. 
Either there is at least one character in the buffer;
that is to say, the field \ml+in_pos+ is less than the field 
\ml+in_end+. We then return the next character in the buffer, 
the one located at \ml+in_pos+, and we increment \ml+in_pos+. Or the
buffer is empty and we call \ml+read+ to refill the 
buffer. If \ml+read+ returns zero, we have reached 
the end of the file and we raise the exception \ml+End_of_file+. Otherwise,
we put the number of characters read in the field \ml+in_end+ (we may
receive less characters than we requested and thus the buffer may be
...
\draw[dashed] (4,3) to node [left,text width=2.5cm,text centered] 
 {written, but bufferized, characters} (4,0);
...
requested. However the requested position can be located after the end
of file. After such a positioning, a \ml+read+ returns zero (end of
file reached); a \ml+write+ extends the file with zero until the
requested position and then writes the supplied data.
...
sockets) but also most special files (peripherals) like the terminal.
...
\ml+read+ returns data we display it immediatly and start again. If it
...
name or by a file descriptor). The second argument is the desired
...
link points. Examples are: \indexvalue{openfile},
\indexvalue{stat}, \indexvalue{truncate}, \indexvalue{opendir}. 
...
The latter, typically disks, have a permanent medium: characteres can
...
tape; for a terminal, choice of the line editing mode, behaviour of
special characters, serial connection parameters (speed, parity,
\etc).  These operations are made in Unix through the system call
...
\subsection*{Terminals\label{sec/termio}}
...
of the terminal associated to \ml+stdin+. Then it defines a modified
...
line. Otherwise we display a message, change the terminal, read the
...
\end{example}
% 
Sometimes a program needs to start another and bind its standard input
...
allows to prevent this. This is fine for \ml+log+ files but it doesn't
work for files that store, for example, a database as writes are
...
or its parent) and recursively call \ml+copy+ for each entry. All
...
\begin{exercise} 
\label{ex/copyrec}
Copy hard links cleverly. As written above \ml+copyrec+ creates $n$
...
hiearachy into a single file. It can be seen as a mini file system.
...
and reread many years leater, the header contains a \ml+checksum+
field which allows to detect when the header is damaged. Its value is
the sum of all the bytes of the header (to make that sum we assume
that the \ml+checksum+ field itself is made of zeros).

The \ml+kind+ header field encodes the file type in a byte. The encoding
is given in the table below\footnote {This field can 
...
the last occurence takes precedence over the other.
%
\begin{listingcodefile}{readtar_ex.ml}
let root () = 
  let rec i = 
...
should be detected and prohibited. Inexistant directories not explicitely
...
processes a single record and accumulates directories explicitely
...
\end{exercise}
\begin{answer}
We reuse the data structures already defined above and collect them 
in a \ml+Tarlib+ module. 
We re-utilize the data structure defined below in the library 
\ml+Tarlib+. We define a warning function which does not stop the
program or alter the return code of the program.  
%
...
\end{listingcodefile}
%
The following function create a record header for a file. \ml+source+ is
...
in the archive are stored in hashtable with their path so that they
are not copied more than once. We also store the directories that were
already written so as to copy them again: it can happen that the
