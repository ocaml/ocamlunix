...
\item standard files: finite sets of bytes containing 
      text or binary information which we may call \quotes{ordinary} files
...
\end{itemize}
%
The concept of a file simultaneously encompasses the data 
...
root directory is represented by \ml+'/'+ . The branches are labeled by names (of  
...
The non-terminal nodes of the file system are
called \emph{directories}: these nodes always contain two branches \ml+.+
and {..} which respectively represent the directory itself and the directory's
parent. The other nodes are sometimes called \quotes{files},
as opposed to directories, but this is ambiguous, as we can also  
designate any node as a \quotes{file}. To avoid all ambiguity 
we refer to them as \quotes{non-directory files}.
...
under Windows is \ml+'\'+).  Similarly, the \ml+Filename+
module provides the string values \libvalue{Filename}{current\_dir\_name} and
\libvalue{Filename}{parent\_dir\_name} to represent the branches
\ml+.+ and \ml+..+.  The functions \libvalue{Filename}{basename} and
\libvalue{Filename}{dirname} retrieve the prefix \ml+d+ and
...
In fact, the file hierarchy is not strictly a tree.  In particular, the
directory conventions \ml+.+ and \ml+..+ allow the directory to refer
to itself (auto-referencing) and allow remounting a directory within the file
...
non-directory file which contains a path.  In addition, a symbolic
link can be interpreted as an ordinary file and we can simply read
what it contains, the path of the linked file.  It is also possible to follow the symbolic
...
%
\ml+link f1 f2+ create a hard link named \ml+f2+ to the file named  
...
information like the current read/write position in the file,
the permissions of the file (read permission, write permission \etc)
and the wrappers which control the behavior of the read and write
process (writes appending or overwriting blocking reads or
...
descriptor file is obtained, the file may be destroyed or renamed
but the descriptor will still point to the original file.
...
and returns information about the specified file. The
difference between  \ml+stat+ and \ml+lstat+ can be seen in symbolic links
: \ml+lstat+  returns information about the symbolic link itself
...
in a portable manner through the help of the functions \syscall{getpwnam} and 
\syscall{getgrnam} or numerically through the help of the functions 
...
use \syscall{fchown} through the descriptor instead of \ml+chown+
on the file name.
%
\begin{listingcodefile}{tmpunix.mli}
...
looked up and changed by the system call \syscall{umask}:
%
\begin{listingcodefile}{tmpunix.mli}
...
directory (or raises the exception \ml+End_of_file+ if the end
of the directory is reached). The returned string consists of the name of the file
relative to the read directory. \syscall{rewinddir} repositions the 
descriptor to the beginning of the directory and \syscall{closedir} closes the
...
\ml+Misc+ module which iterates over the entries in a directory.
%
...
specifically of type \ml+Unix_error+.  Exception arguments
...
(line~\ref{prog:did}) which consists of its device number and
its inode. The list \ml+visiting+ contains all the directories
...
the file should be created with if that case occurs.  The result is a
...
\end{mltypecases}
% 
The first group defines the following behavior according to whether
the file exist or not.
\begin{itemize}
\item \ml+O_APPEND+ specifies that the read/write cursor 
should be positioned at the end of the file before the first write,
and that all the writes are appended to the end 
of the file. Without  \ml+O_APPEND+, the writes occur
at the current position (initially, the beginning of the file).  

\item \ml+O_TRUNC+ specifies that the file is truncated when it
...
\item \ml+O_CREAT+ specifies that if the file does not already exist, 
the file should be created (initially with zero size), and with access rights
...
\item \ml+O_EXCL+ specifies that \ml+openfile+ should fail if the file 
already exists.  This flag, when used in conjunction with \ml+O_CREAT+
allows the use of files as (\emph{locks}).
%
\footnote{This is not possible if the lock file is located on a
  \textsc{nfs} partition, because \textsc{nfs} does not implement the
  option \ml+O_CREAT+ of \ml+open+.}
%
correctly.  A process which wants to take the lock calls \ml+openfile+
on the file with the modes \ml+O_EXCL+ and \ml+O_CREAT+. If the file
already exists, this signifies that another process has the lock.  In
this case, \ml+openfile+ triggers an error and it is necessary to wait
a little and then re-try. If the file does not exist \ml+openfile+
returns without error and the file is created. preventing the other
processes from taking the lock. To release the lock the process that
has it calls \ml+unlink+ on it. The creation of a file is an atomic
operation. If two processes attempt to create the same file in
parallel with the options \ml+O_EXCL+ and \ml+O_CREAT+, only one of
the two can carry it out.  Clearly, this method is not very
satisfactory because for one part the process that does not have the
lock has to wait.  For another, a process may terminate abnormally and
leave the lock blocked.
\end{itemize}
...
synchronized.
\begin{itemize}
\item\ml+O_DSYNC+ specifies that data is written 
...
\item\ml+O_SYNC+ specifies that the file data and the information about the file 
are synchronized.

\item\ml+O_RSYNC+ when used with \ml+O_DSYNC+ specifies that the data
reads are also synchronized. It is guaranteed that all current writes
(requested but not necessarily 
registered) to the file are actually written to the media before the
...
to be written or read. The fourth argument is the number of the bytes to be read or written.
...
exceptions to this behavior: (i) In the case where it is not possible
...
Firstly, when the end of a file
...
Secondly, the number of bytes read may be less than the number of bytes
...
(This is the default behavior of a terminal;
...
\ml+read+, but makes additional read attempts if necessary to try to
get the requested number of bytes. It raises the exception
\ml+End_of_file+ if it encounters the end of the file while doing
this.
...
The special files have a behavior sufficient variable in response
...
system call is not provided by {\ocaml}; it is  
...
As tar archives are designed to be written to unreliable media
...
use more than one file as an archive.) The checksum value is the sum of the character
