...
The figure~\ref{fig/hierarchy} gives an example of a file hierarchy
...
\indexlibvalue{Unix}{chroot}\ml+ p+ makes the node \ml+p+, which
...
non-blocking).  The descriptors are represented by by values of type
...
a lockfile placed in non-blocking input/output mode, the write
